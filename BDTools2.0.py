...
def main():
    ...
    group1.add_argument('-out_num_pop', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    group2.add_argument('-out_por_pop', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    #常住数量
    group3 = parser.add_argument_group('常住数量', '四类常住人口数量（居住，工作，居住不工作，工作不居住）', gooey_options={"columns": 1})
    ...
    group3.add_argument('-out_num_stay', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    group4.add_argument('-out_por_stay', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    group5.add_argument('-out_OD', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    group6.add_argument('-out_num_lw', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    group7.add_argument('-out_time_lw', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    group8.add_argument('-out_way_lw', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    group9.add_argument('-out_por_lw', metavar='结果文件保存路径', help="建议保存为csv格式", widget="FileSaver", nargs='?')
    ...
    if args.num_pop and args.num_pop_geo:
        ...
        print('空间相交完成!')
        dfb.to_csv(args.out_num_pop, encoding='UTF-8')
    ...
    if args.por_pop and args.por_pop_geo:
        ...
        dfb = intersect(df, dfy)
        print('空间相交完成!')
        dfb.to_csv(args.out_por_pop, encoding='UTF-8')
    ...
    if args.num_stay and args.num_stay_geo:
        ...
        if args.lw_ratio and args.opt4:
            calc_ratio(dfb)
        dfb.to_csv(args.out_num_stay, encoding='UTF-8')
    ...
    if args.por_stay and args.por_stay_geo:
        ...
        dfb = intersect(df, dfy)
        print('空间相交完成!')
        dfb.to_csv(args.out_por_stay, encoding='UTF-8')
    ...
    if args.num_OD:
        ...
        print('空间相交完成!')
        dfb.to_csv(args.out_OD, encoding='UTF-8')
    ...
    if args.num_lw:
        ...
        #两个范围
            ...
            dfb = D_intersect(temp, dfy2)    
        print('空间相交完成!')
        dfb.to_csv(args.out_num_lw, encoding='UTF-8')
    ...
    if args.time_lw:
        ...
        #两个范围
            ...
            dfb = D_intersect(temp, dfy2)    
        print('空间相交完成!')
        dfb.to_csv(args.out_time_lw, encoding='UTF-8')
    ...
    if args.way_lw:
        ...
        #两个范围
            ...
            dfb = D_intersect(temp, dfy2)    
        print('空间相交完成!')
        dfb.to_csv(args.out_way_lw, encoding='UTF-8')
    ...
    if args.por_lw:
        ...
        #两个范围
            ...
            dfb = D_intersect(temp, dfy2)    
        print('空间相交完成!')
        dfb.to_csv(args.out_por_lw, encoding='UTF-8')
...
def calc_ratio(df):
    ...
    if df.columns.__contains__('居住人数'):
        ratio = df['人数'].sum()/df['居住人数'].sum()
    elif df.columns.__contains__('工作人数'):
        ratio = df['工作人数'].sum()/df['人数'].sum()
